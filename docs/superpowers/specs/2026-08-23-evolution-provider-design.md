...
> **Nota (2026-09-08):** o provider Evolution foi desligado da fiação em 2026-08-27 e nunca entrou em produção. `provider.evolution.ts` e seus testes seguem no repositório, mas `getWhatsappProvider` só liga `fake` e `uazapi`. Ver `docs/ops/whatsapp-provider-evolution.md`. Documento mantido como registro do desenho.
