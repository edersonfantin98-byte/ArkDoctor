...
- **Despesas parceladas** (Financeiro → Novo lançamento → despesa → "Parcelar") — uma compra de N parcelas (2–48) gera N lançamentos de despesa, um por mês, com `occurred_at` = vencimento; cada parcela só entra nos totais do mês dela (sem status pendente/pago). O detalhe da compra (total, já vencido, restante) e a exclusão em lote (futuras ou todas) ficam no diálogo de edição da parcela. Ver `docs/superpowers/specs/2026-09-18-despesas-parceladas-design.md`.
